...
def fetch(url):
    try:
        response = requests.get(PROXY_POOL_URL, timeout=50)
...
if __name__ == '__main__':
    #movies = pd.read_csv('./failed682.csv')
    movies = pd.read_csv('./reviews_url.csv')
    #movies['comment_href'] = movies['href'] + 'comments?status=P'
    ...
    time.sleep(10)
    df = pd.DataFrame(list(table.queue), columns=['name', 'rating', 'href','comment_href', 'comment_nums'])
